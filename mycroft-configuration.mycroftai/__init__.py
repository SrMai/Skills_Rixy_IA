    ...
    def decorator(function):
        def wrapper(self, message):
            try:
                ...
                self.log.exception('En función envuelta segura')
...
class ConfigurationSkill(MycroftSkill):
    ...
    def handle_query_name(self, message):
        ...
        self.speak_dialog("mi.nombre.es", data={"name": device["name"]})
    ...
    def handle_disable_precise_dev(self, message):
        ...
        self.bus.emit(Message('configuration.updated'))
        self.speak_dialog('precisa.devmode.deshabilitado')
    ...
    def handle_update_precise(self):
        ...
        if isfile(self.model_file):
            ...
            self.speak_dialog('modelos.no.encontrados')
    ...
    def update_remote(self, message):
        ...
        try:
            if self.update():
                self.log.info('Configuración remota actualizada')
        except Exception as e:
            if isinstance(e, HTTPError) and e.response.status_code == 401:
                self.log.warn("Imposible actualizar la configuración porque"
                               "el dispositivo no está emparejado")
            else:
                self.log.warn("No se pudo actualizar la configuración, se volverá a intentar más tarde")
