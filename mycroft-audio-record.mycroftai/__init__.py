...
class AudioRecordSkill(MycroftSkill):
    ...
    def handle_delete(self, message):
        if not exists(self.settings["file_path"]):
            self.speak_dialog('grabación.de.audio.sin.grabacion')
        else:
            try:
                os.remove(self.settings["file_path"])
                self.speak_dialog('registro.de.audio.eliminado')
            except Exception:
                pass
    ...
    def handle_record(self, message):
        ...
        if self.has_free_disk_space():
            record_for = nice_duration(self.settings["duration"],
                                       lang=self.lang)
            self.speak_dialog('grabación.de.audio.inicio.duración',
                              {'duration': record_for})
            ...
            self.speak_dialog("disco.de.grabación.de.audio.lleno")
    ...
    def handle_play(self, message):
        if exists(self.settings["file_path"]):
            ...
            self.speak_dialog('grabación.de.audio.sin.grabación')
