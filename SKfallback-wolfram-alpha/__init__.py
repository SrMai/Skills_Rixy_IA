...
class WolframAlphaSkill(CommonQuerySkill):
    ...
    def on_settings_changed(self):
        self.log.debug("ajustes cambiados")
        self._setup()

    def _setup(self):
        self.autotranslate = self.settings.get("autotranslate", True)
        self.log.debug("autotraducir: {}".format(self.autotranslate))
    ...
    def CQS_match_query_phrase(self, utt):
        self.log.debug("Consulta WolframAlpha: " + utt)
        ...
        if self.autotranslate and self.lang[:2] != "en":
            ...
            self.log.debug("traducción: {}".format(utt))
        ...
        if parsed_question:
            ...
            self.log.debug("Consultando WolframAlpha: " + query)
        else:
            # This utterance doesn't look like a question, don't waste
            # time with WolframAlpha.
            self.log.debug("Sin preguntas, ignorando: " + utterance)
            return False
    ...
    def CQS_action(self, phrase, data):
        ...
        if data:
            self.log.info("Configuración de la información para la fuente")
