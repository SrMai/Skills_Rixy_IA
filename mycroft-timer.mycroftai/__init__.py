...
class TimerSkill(MycroftSkill):
    ...
    def initialize(self):
        ...
        if self.active_timers:
            self.log.info("encontrado {} Temporizadores activos".format(str(len(self.active_timers))))
            self._show_gui()
    ...
    def _request_duration(self) -> timedelta:
        ...
        if response is None:
            raise TimerValidationException("No hay respuesta a la solicitud de duración del temporizador..")
        else:
            duration, _ = extract_timer_duration(response)
            if duration is None:
                raise TimerValidationException("No se especifica ninguna duración")
    ...
    def _handle_duplicate_name_error(self, duplicate_timer: CountdownTimer):
        ...
        raise TimerValidationException("Ya existe el nombre del temporizador solicitado")
    ...
    def _speak_timer_status_matches(self, matches: List[CountdownTimer]):
        ...
        if matches:
            ...
            if number_of_timers > 1:
                ...
                self.speak_dialog("numero-de-temporizadores", dialog_data)
    ...
    def _cancel_timers(self, message: Message):
        ...
        if not self.active_timers:
            self.speak_dialog("temporizador-no-activo")
        elif cancel_all:
    ...
    def _cancel_all_timers(self):
        ...
        if len(self.active_timers) == 1:
            self.speak_dialog("temporizador-unico-cancelado")
        else:
            self.speak_dialog("cancelar-todo", data={"contar": len(self.active_timers)})
    ...
    def _determine_which_timer_to_cancel(self, utterance: str):
        ...
        if matches:
            ...
            self.speak_dialog("temporizador-no-encontrado")
    ...
    def _start_display_update(self):
        ...
        if self.active_timers:
            self.log.info("comenzando un evento repetitivo para actualizar la pantalla del temporizador")
            if self.platform == MARK_I:
                self.enclosure.mouth_reset()
    ...
    def _stop_display_update(self):
        ...
        self.log.info("detener la repetición de eventos para actualizar la pantalla del temporizador")
        self.cancel_scheduled_event("UpdateTimerDisplay")
        if self.platform == MARK_I:
            self.enclosure.mouth_reset()
    ...
    def _start_expiration_check(self):
        ...
        if self.active_timers:
            self.log.info("comenzando un evento repetitivo para verificar la expiración del temporizador")
            self.schedule_repeating_event(
    ...
    def _stop_expiration_check(self):
        ...
        self.log.info("detener el evento repetitivo para verificar la expiración del temporizador")
        self.cancel_scheduled_event("ExpirationCheck")
