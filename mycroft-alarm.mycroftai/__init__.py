...
class AlarmSkill(MycroftSkill):
    ...
    def __init__(self):
        ...
        try:
            ...
            try:
                ...
                self.log.error("No se pudo asignar el mezclador, {}".format(repr(err)))
                self.mixer = None
    ...
    def set_alarm(self, when, name=None, repeat=None):
        ...
        for existing in self.settings["alarm"]:
            if alarm == existing:
                self.speak_dialog("alarma.ya.existe")
                return None
    ...
    def handle_set_alarm(self, message):
        ...
        if (when is None or when.time() == today.time()) and not is_midnight:
            ...
            if not response:
                self.speak_dialog("horario.de.alarma.cancelado")
                return
    ...
    def _get_alarm_name(self, utt):
        ...
        self.log.debug("Expresión buscada: " + utt)
        ...
        if utt and rx_file:
            ...
            for pat in patterns:
                self.log.debug("Patrón de expresiones regulares: {}".format(pat))
                res = re.search(pat, utt)
                if res:
                    try:
                        name = res.group("Name").strip()
                        self.log.debug("Nombre Regex extraído: {}".format(name))
    ...
    def handle_status(self, message):
        ...
        if len(self.settings["alarm"]) == 0:
            self.speak_dialog("alarmas.lista.vacío")
            return
    ...
    def _get_alarm_matches(
        ...
        if alarms is None or len(alarms) == 0:
            self.log.error("No se puede obtener coincidencia. Sin alarmas activas.")
    ...
    def handle_delete(self, message):
        ...
        if total == 1:
            ...
            if (
                ...
                self._schedule()
                self.speak_dialog(
                    "alarma.cancelada.desc" + recurring, data={"desc": desc}
                )
                self.gui.release()
                return
            else:
                self.speak_dialog("alarma.eliminar.cancelado")
            ...
            if (
                ...
                self._schedule()
                self.speak_dialog("alarma.cancelada.multi", data={"count": total})
                self.gui.release()
            return
        elif not total:
            # Failed to delete
            self.speak_dialog("alarma.no.encontrada")
    ...
    def handle_change_alarm(self, _):
        ...
        """
        self.speak_dialog("alarma.cambiar.sonido")
