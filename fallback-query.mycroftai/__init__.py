...
class QuestionsAnswersSkill(FallbackSkill):
    ...
    def handle_question(self, message):
        """ Envíe la frase a CommonQuerySkills y prepárese para el manejo
            las respuestas
        """
        self.waiting = True
        self.answered = False
        ...
        self.log.info('Buscando {}'.format(utt))
        ...
        with self.lock:
            ...
            if searching:
                ...
                if answer and search_phrase in self.query_replies:
                    self.log.info('Respuesta de {}'.format(skill_id))
                ...
                self.log.warning('{} Respondió demasiado lento, '
                                  'será ignorado.'.format(skill_id))

    def _query_timeout(self, message):
        # Prevent any late-comers from retriggering this query handler
        with self.lock:
            self.log.info('Se agotó el tiempo de espera comprobar las respuestas')
            search_phrase = message.data['phrase']
            ...
            if best:
                ...
                self.log.info('Manejo con: ' + str(best['skill_id']))
